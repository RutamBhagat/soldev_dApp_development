...
import { Metaplex, NftWithToken, bundlrStorage, keypairIdentity, toMetaplexFile } from "@metaplex-foundation/js";
...
async function createCollectionNft(metaplex: Metaplex, uri: string, data: CollectionNftData): Promise<NftWithToken> {
  const { nft } = await metaplex.nfts().create(
    {
      uri: uri,
      name: data.name,
